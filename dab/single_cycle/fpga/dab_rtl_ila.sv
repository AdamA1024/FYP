...
module dab_rtl #(
    ...
    //   dt =  20 ns,  fsw = 500 kHz  (HALF_PERIOD = 50 timesteps)
    //   V1 = 400 V,   N = 2,  V2_nom = 200 V
    //   L  =  20 uH  (transformer leakage inductance, sized for fsw=500 kHz)
    //   Co = 100 uF,  R = 10 ohm  (output filter / load)
    //
    // All constants are stored as round(real_value x 2^FRAC).
    //
    // alpha = dt/(2L) = 20e-9/(2x20e-6)  = 5.000e-4  ->  round(5e-4 x 2^20) = 524
    parameter logic signed [WIDTH-1:0] ALPHA    = 32'sd524,

    // beta = dt/Co: 20e-9/100e-6 = 2e-4 -> round(2e-4 x 2^20) = 210.
    // GAMMA/BETA = 21/210 = 0.1 = 1/R exactly -> zero DC quantisation error.
    parameter logic signed [WIDTH-1:0] BETA     = 32'sd210,

    // gamma = dt/(R*Co)= 20e-9/(10x100e-6) = 2.000e-5  ->  round(2e-5 x 2^20) = 21
    parameter logic signed [WIDTH-1:0] GAMMA    = 32'sd21,

    // delta = R_L*dt/(2L)  (inductor series resistance, applied as half-step each side)
    //   R_L ~ 0.1 ohm  ->  delta = 0.1x20e-9/(2x20e-6) = 5.000e-5  ->  round(x2^20) = 52
    ...
    parameter logic signed [WIDTH-1:0] DELTA_L  = 32'sd52,
    ...
    (* mark_debug = "true" *) input logic p1,
    (* mark_debug = "true" *) input logic p2,
    ...
    // State registers
    (* mark_debug = "true" *) logic signed [WIDTH-1:0] i_L_reg;
    (* mark_debug = "true" *) logic signed [WIDTH-1:0] V2_reg;
    ...
    //
    // Round-half-up (add 1/2 LSB before right-shift) instead of truncation.
    // Plain truncation always rounds toward zero: for always-positive quantities
    // like V2, fpmul(GAMMA, V2) is consistently ~1/2 LSB too small every cycle,
    // leaving a tiny net positive power imbalance that manifests as a slow upward
    // drift in V2 steady state.  Rounding removes that systematic bias.
    function automatic logic signed [WIDTH-1:0] fpmul (
        ...
        prod = a * b;
        // Add 1/2 LSB (2^(FRAC-1)) before arithmetic right-shift to round-half-up.
        return WIDTH'((prod + (2*WIDTH)'(1 << (FRAC-1))) >>> FRAC);
    endfunction
